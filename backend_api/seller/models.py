...
class seller_products(models.Model):
    ...
    def save(self, *args, **kwargs):
        if self.offer_end and self.offer_end < timezone.now().date():
            self.offer_price = 0
            self.offer_start = None
            self.offer_end = None
...
class Order(models.Model):
    ...
    location = models.ForeignKey(
        "user.location", on_delete=models.SET_NULL, null=True, blank=True
    )
...
class Payment(models.Model):
    ...
    order = models.ForeignKey(
        Order, on_delete=models.DO_NOTHING, related_name="payment_orders"
    )
