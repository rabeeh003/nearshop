        ...
        if (oldStatus === "Returned") {
          ...
          for (const product of returnedProducts) {
            try {
              ...
              await axios.delete(`https://www.nearbazar.shop/api/s/orderproduct/${product.id}/`);
              console.log(`Product ${product.id} deleted successfully`);
            } catch (error) {
              console.error(`Error deleting ordered product ${product.id}:`, error);
            }
          }
          axios.put(`https://www.nearbazar.shop/api/customer/${returnedProducts[0].user}/`, { wallet: returnPrice })
        }
        if (oldStatus === "Replace") {
          const returnedProducts = proForThis.filter(product => product.returned === true);
          for (const product of returnedProducts) {
            try {
              await axios.put(`https://www.nearbazar.shop/api/s/orderproduct/${product.id}/`, { returned: false });
...
function ReturnModel({ show, onHide, product, user }) {
  ...
  const returnOrder = () => {
    ...
    axios.put(`https://www.nearbazar.shop/api/s/orders/${product[0].order}/`, updatedData)
      .then(async response => {
