...
function EditShopInfo() {
    ...
    const fetchShopData = async (shopId) => {
        try {
            const response = await axios.get(`https://www.nearbazar.shop/api/getOneShop/${shopId}/`); // Replace '1' with the shop ID you want to retrieve
    ...
    const handileSibmit = async (event) => {
        ...
        if (formDataToSend !== null) {
            try {
                ...
                await axios.put(`https://www.nearbazar.shop/api/shop_update/${shopData.id}/`, formDataToSend, {
                    headers: {
