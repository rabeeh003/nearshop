...
function OrderHis(props) {
    ...
    const handleInvoiceClick = (orderData,thisOrderProducts) => {
        console.log("start", orderData);
    ...
    useEffect((orderHistory) => {
        const fetchData = async () => {
            try {
                const res = await axios.get("https://www.nearbazar.shop/api/s/orders/");
                const userOrder = res.data.filter(order => order.user === props.userId && order.status === "Delivered");
                setOrderHistory(userOrder);
                try {
                    const responce = await axios.get('https://www.nearbazar.shop/api/s/orderproduct/');
                                        ...
                                        <Row className=''>
                                            ...
                                            <Col className='d-flex justify-content-end m-3 align-items-center'>
                                                {/* <Link className='text-reset text-decoration-none m-2'> */}
                                                    <ActioinBt
                                                        onClick={() => handleInvoiceClick(orderHistory[idx],thisOrderProducts)}
